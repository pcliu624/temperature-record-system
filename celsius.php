...
<link href="web.css?version=$lt:?php echo time(); ?&gt;" rel="stylesheet">
...
<title>Smart home</title>
  ...
  <div class="container">
      <?php
      $sql = "SELECT * FROM(SELECT * FROM celsius ORDER BY datetime DESC LIMIT 1) AS c ORDER BY datetime ASC;";
      $data = $dbhandle->query($sql);
      if($data->num_rows>0){
        while($row = $data->fetch_assoc()) {
          echo "<h3>"."Current TEMP: ".$row["celsius"]."°C"."</h3>"."<br>";
      }
    }
 ...
 $strQuery = "SELECT * FROM(SELECT * FROM celsius ORDER BY datetime DESC LIMIT 24) AS c ORDER BY datetime ASC; ";
 ...
 $arrData = array(
     ...
     // chart object
     $msChart = new FusionCharts("mscombi2d", "chart" , "960", "630", "chart-container", "json", $jsonEncodedData);
     // Render the chart
     $msChart->render();
