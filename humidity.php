...
<title>Smart home</title>
  ...
  <body  id="myPage" data-spy="scroll" data-target=".navbar" data-offset="50">
    ...
    <div class="container">
      <?php
      $sql = "SELECT * FROM(SELECT * FROM humidity ORDER BY datetime DESC LIMIT 1) AS c ORDER BY datetime ASC;";
      $data = $dbhandle->query($sql);
      if($data->num_rows>0){
        while($row = $data->fetch_assoc()) {
          echo "<h3>"."Current humidity: ".$row["humidity"]."%"."</h3>"."<br>";
      }
    }

 $strQuery = "SELECT * FROM(SELECT * FROM humidity ORDER BY datetime DESC LIMIT 24) AS c ORDER BY datetime ASC; ";
